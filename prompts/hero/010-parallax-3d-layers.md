...
title: Parallax 3D layers ¡ª depth hero
category: Hero
type: hero
access: open
...
# Parallax 3D layers ¡ª depth hero
...
```text
Build a one-screen hero for "Atlas ¡ª map your ideas". Background: warm off-white #f5efe6. Stack 4 absolutely-positioned layers inside a 100vh container, each layer containing 1 SVG / PNG illustration. Layer 0 (background): soft radial gradient blob (#ffbf69 alpha .25, blur 60px). Layer 1 (mid): vector mountains, blurred 4px, translateY ¡Ü 12px on pointermove. Layer 2 (hero object): a stylized compass icon, translateX/Y ¡Ü 24px and rotateZ ¡Ü 4¡ã. Layer 3 (foreground): 6 floating dots/particles, each with its own micro-animation (translateY ¡À6px loop) and a 200ms delay. Use `transform: translate3d(...)` on each layer; bind pointer position to a single `transform` reflow per `requestAnimationFrame`. On scroll past the hero, gently scale layer 2 down to 0.92 over 600ms.
```
